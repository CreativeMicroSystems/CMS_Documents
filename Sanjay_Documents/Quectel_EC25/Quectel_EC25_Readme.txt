...
------------------------ FIRMWARE + KERNEL GENERATION BEGIN ------------------
	...
	Step 3: Make a copy of the "rootfs" dir as a backup
			-> cp -r ql-ol-rootfs ql-ol-rootfs_ORG
	Step 4:	Transfer the .lib, .bin, necessary files you needed to be in the RootFS to the "ql-ol-rootfs" folder
			-> cp <.BIN_File_path> ql-ol-rootfs/bin
			-> cp <.LIB_File_path> ql-ol-rootfs/lib
	Step 5: Give the necessary permissions
			-> chmod +x ql-ol-rootfs/bin/<.BIN_File_path> 
			-> chmod +x ql-ol-rootfs/bin/<.LIB_File_path>
	Step 6:	Transfer the CMS related files you needed to be in the /usrdata to the "ql-ol-usrdata" folder
			-> mkdir -p ql-ol-usrdata/cms/bin
			-> mkdir -p ql-ol-usrdata/cms/lib
			-> mkdir -p ql-ol-usrdata/cms/config
			-> cp <.BIN_File_path> ql-ol-usrdata/cms/bin
			-> cp <.LIB_File_path> ql-ol-usrdata/cms/lib
			-> cp <.CFG_File_path> ql-ol-usrdata/cms/config
	Step 7: Give the necessary permissions
			-> chmod +x ql-ol-usrdata/cms/bin/*
			-> chmod +x ql-ol-usrdata/cms/lib/*
			-> chmod +x ql-ol-usrdata/cms/config/*
	Step 8: Give the necessary permissions
			-> chmod +x ql-ol-rootfs/bin/<.BIN_File_path> 
			-> chmod +x ql-ol-rootfs/bin/<.LIB_File_path>
	Step 9: Defaultly during firmware generation it will create folder named "target" for storing the generated firmware files so removing the folder
			-> rm -rf $KERNEL_SDK_PATH/target
	Step 10: Cleaning the already build things
			-> make clean
				(or)
			For specifically cleaning rootfs
				-> make rootfs/clean
			For specifically cleaning usrdata
				-> make usrdata/clean
	Step 11: Making the files
			-> make
				(or)
			For specifically Making rootfs
				-> make rootfs
			For specifically Making usrdata
				-> make usrdata
	Step 12: The new folder named "target" got created and it contains all the compiled firmwares + rootFS
			(eg : mdm9607-perf-sysfs.ubi, usrdata.ubi, usrdata.ubifs, So on...)
			-> ls target
	#IMPORTANT NOTES
		The Linux system will run the /etc/rcS startup file first. Afterward, it will mount the /usrdata folder.
		Therefore, before executing any files located in the /usrdata folder, there must be a delay between the execution of the /etc/rcS file and the mounting of /usrdata.
------------------------ FIRMWARE + KERNEL GENERATION END --------------------
...
------------------ TEST QUECTEL EC25-E with QUECTEL FC20-Q73 -----------
	Note : The Quectel FC20-Q73 WIFI module uses the Qualcomm QCA6174 chipset inside it.
	Note : In the vendor's default Kernel the "Quectel FC20-Q73" is enabled by default.

	************ QCA6174 Driver in Menuconfig BEGIN ************
	...
	************ QCA6174 Driver in Menuconfig END ************

	step 1: Flash the default kernel given by vendor via QFlash to the unit
...
******************* DNSMASQ NOTE BEGIN *******************
	...
	After enabling wifi the api will merge both AP+AP interfaces wlan0 and waln1 in a single bridge => bridge0 ,
		This effectively combines the two interfaces treating them as a single logical interface for IP assignment
